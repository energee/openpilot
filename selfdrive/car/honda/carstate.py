...
# this function generates lists for signal, messages and initial values
  ...
  elif CP.carFingerprint == "HONDA CR-V 2017 EX":
    dbc_f = 'honda_crv_ex_2017_can.dbc'
    signals = [
      ("XMISSION_SPEED", 0x158, 0),
      ("XMISSION_SPEED2", 0x158, 0),
      ("WHEEL_SPEED_FL", 0x1d0, 0),
      ("WHEEL_SPEED_FR", 0x1d0, 0),
      ("WHEEL_SPEED_RL", 0x1d0, 0),
      ("STEER_ANGLE", 0x14a, 0),
...
#       ("HUD_LEAD", 0x30c, 0),
  ...
  elif CP.carFingerprint == "HONDA CIVIC HATCHBACK 2017 EX":
...
class CarState(object):
  ...
  def update(self, cp):
    ...
    if self.crv5g:
      self.v_cruise_pcm = 0
      self.hud_lead = 0
      self.acc_on = cp.vl[0x1EF]['CONTROL_ON']
    elif self.civichatch:
      self.v_cruise_pcm = 0
      self.hud_lead = 0
      self.acc_on = cp.vl[0x1EF]['CONTROL_ON']
